...
class NN():
    ...
    def batch_train(self, X, Y, inpt_size, otpt_size, batch_size):
        init_value = 0
        index_x = 0
        index_y = 0
        lay = len(self.layers)
        ...
        for i in range(batch_size):
            self.backward(X[index_x:index_x+inpt_size], Y[index_y:index_y+otpt_size])
            index_x = index_x + inpt_size
            index_y = index_y + otpt_size
...
if __name__ == '__main__':
    ...
    for i in range(1000):
        model.batch_train(X, Y, 3, 2, 8)
        print(model.forward([1, 9, 3]))
        #print(model.delta['d_2'])
